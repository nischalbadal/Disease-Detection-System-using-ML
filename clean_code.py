...
# defining a list for user inputs
recordList = []
...
def DecisionTree():
    ...
    dacc = accuracy_score(y_test, y_pred, normalize=False)
    ...
    if (h == 'yes'):
        ...
        finalpredict = disease[a]
    else:
        t1.delete("1.0", END)
        t1.insert(END, "Not Found")
    # writing to csv
    data_file = open("data.csv", 'a')
    data_file.write("\n Name of Patient: "+name+"\n Decision Tree Prediction \n"+"Symptoms \t "+str(psymptoms) +
...
def randomforest():
    ...
    racc = accuracy_score(y_test, y_pred, normalize=False)
...
def NaiveBayes():
    ...
    nacc = accuracy_score(y_test, y_pred)
    ...
    # writing to csv
    data_file = open("data.csv", 'a')
    data_file.write("\n Naive Bayes Prediction \t"+finalpredict)
...
# Heading
w2 = Label(root, text="eCheckUp - Disease Predicton System using Machine Learning",
           fg="red", bg="white")
...
# accur = Label(root, text="Accuracy", fg="black",
#               bg="white", COMMAND=print(nacc))
# accur.grid(row=12, column=0, pady=2, sticky=W)


lrLb = Label(root, text="Prediction 1", fg="white", bg="red")
lrLb.grid(row=14, column=0, pady=8, sticky=W)
...
lr.grid(row=19, column=1, padx=10)
...
t3 = Text(root, height=1, width=40, bg="orange",
          fg="black", command=print(dacc))
t3.grid(row=18, column=1, padx=10)


# calling export to CSV function
sv = Button(root, text="Export to CSV", bg="red", fg="black", padx=80, pady=10)
sv.grid(row=21, column=0)
